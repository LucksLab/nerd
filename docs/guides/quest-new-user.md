...
Every line should report success. If one fails, read its message and consult
...
It prints two identifiers:

- The **task ID** is NERD's number for the analysis.
- The **Slurm job ID** is Quest's number for the submitted job.

Write down the task ID. You may close Terminal after submission; the Quest job
will continue.
