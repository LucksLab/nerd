...
class NmrDegKineticsTask(Task):
    """
    Run exponential degradation (kdeg) fits for NMR reactions.
    """

    name = "nmr_deg_kinetics"
    scope_kind = "nmr_batch"
    _DEFAULT_TRACE_ROLES = {"decay_trace": "decay_trace"}
    ...
    def prepare(self, cfg: Dict[str, Any]) -> tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        block.setdefault("plugin", "lmfit_deg")
    ...
    def consume_outputs(
        ...
        if not reactions:
            raise ValueError("No NMR degradation reactions matched the provided filters.")
        ...
        model_name = str(inputs.get("model") or plugin_name or "nmr_deg")

        for row in reactions:
            metadata = dict(row)
            metadata.setdefault("species", inputs.get("species") or row["probe"] or "dms")
            ...
            species = str(inputs.get("species") or metadata.get("species") or "dms")
