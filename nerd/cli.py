...
from nerd.exporting import (
    MUT_COUNT_COLUMNS,
    PROBE_TIMECOURSE_COLUMNS,
    mut_count_rows,
    probe_timecourse_rows,
    write_csv,
)
...
export_app = typer.Typer(no_args_is_help=True, help="Export persisted scientific results as CSV.")
...
@export_app.command("mut_count")
def export_mut_count(
    ctx: typer.Context,
    samples: Optional[List[str]] = typer.Argument(
        None, help="One or more sample names. Use --all instead to export every sample."
    ),
    all_samples: bool = typer.Option(False, "--all", help="Export all samples with mut_count results."),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="CSV path; writes CSV to standard output when omitted."
    ),
    db: Optional[Path] = typer.Option(None, "--db", help="SQLite database path."),
    project: Optional[Path] = typer.Option(None, "--project", help="Project root or .nerd/project.toml."),
) -> None:
    """Export per-sample mutation rates and read depths."""
    conn = None
    try:
        conn = _scheduler_connection(
            context=_command_context(ctx, db, project), read_only=True
        )
        rows = mut_count_rows(conn, samples=samples, all_samples=all_samples)
        written = write_csv(rows, MUT_COUNT_COLUMNS, output)
        if written is not None:
            typer.echo("Exported %d rows to %s" % (len(rows), written))
    except (ContextResolutionError, OSError, ValueError) as exc:
        typer.echo("Export failed: %s" % exc, err=True)
        raise typer.Exit(code=1)
    finally:
        if conn is not None:
            conn.close()
...
@export_app.command("probe_timecourse")
def export_probe_timecourse(
    ctx: typer.Context,
    task_id: Optional[int] = typer.Option(
        None, "--task-id", min=1, help="Export fits generated by this probe_timecourse task."
    ),
    rg_ids: Optional[List[int]] = typer.Option(
        None, "--rg-id", min=1, help="Reaction group ID; repeat for multiple groups."
    ),
    all_results: bool = typer.Option(False, "--all", help="Export all persisted reaction-group fits."),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="CSV path; writes CSV to standard output when omitted."
    ),
    db: Optional[Path] = typer.Option(None, "--db", help="SQLite database path."),
    ...
    """Export final probe-timecourse fit parameters."""
    conn = None
    try:
        conn = _scheduler_connection(
            context=_command_context(ctx, db, project), read_only=True
        )
        rows = probe_timecourse_rows(
            conn, task_id=task_id, rg_ids=rg_ids, all_results=all_results
        )
        written = write_csv(rows, PROBE_TIMECOURSE_COLUMNS, output)
        if written is not None:
            typer.echo("Exported %d rows to %s" % (len(rows), written))
    except (ContextResolutionError, OSError, ValueError) as exc:
        typer.echo("Export failed: %s" % exc, err=True)
